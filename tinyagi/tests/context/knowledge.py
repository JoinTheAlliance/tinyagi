from tinyagi.context.knowledge import (
    add_knowledge,
    remove_knowledge,
)
...
def test_add_knowledge():
    ...
    knowledge = search_memory("knowledge", "test")
    assert len(knowledge) > 0 and knowledge[0]["document"] == "test"
    delete_memory("knowledge", not_unique_memories[0]["id"])
    ...
    knowledge = search_memory("knowledge", example_text, max_distance=0.05)
    assert (
        len(knowledge) > 0 and knowledge[0]["document"] == example_text
    ), "Knowledge should be example_text"

    knowledge = search_memory(
        "knowledge", example_text, max_distance=1.0, min_distance=0.1
    )
    assert (
